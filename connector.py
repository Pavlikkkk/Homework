...
class Connect(Configurator):
    ...
    def __init__(self, server, database):
        ...
        try:
            self.server = server
            self.database = database  # 'AdventureWorks2017'
    ...
    def queries_source(self):
        ...
        cursor.execute(
            "select top(3) SCHEMA_NAME(schema_id), name, schema_id from sys.all_objects s where s.type = 'U' and schema_id > 8")
        tables = cursor.fetchall()
        return tables

    # это надо будет переместить так чтоб брал из файла и конфиг-ра в main
...
if __name__ == '__main__':
    x = Connect(r'EPBYGOMW001A\SQL17', 'AdventureWorks2017')
